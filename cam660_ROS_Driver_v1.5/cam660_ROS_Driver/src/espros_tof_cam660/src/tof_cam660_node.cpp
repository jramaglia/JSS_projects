...
#include <espros_tof_cam660/espros_tof_cam660Config.h>
...
void setParameters()
{
    ...
    ROS_INFO("setParameters...");
    ROS_DEBUG("lensType %d", lensType);
    ROS_DEBUG("lensCenterOffsetX %d", lensCenterOffsetX);
    ROS_DEBUG("lensCenterOffsetY %d", lensCenterOffsetY);
    ROS_DEBUG("imageType %d", imageType);
    ROS_DEBUG("startStream %d", startStream);
    ROS_DEBUG("integrationTime0 %d", int0);
    ROS_DEBUG("integrationTime1 %d", int1);
    ROS_DEBUG("integrationTimeGray %d", intGr);
    ROS_DEBUG("minAmplitude %d", minAmplitude);
    ROS_DEBUG("offset %d", offset);
    ROS_DEBUG("frequency_modulation %d", frequencyModulation);
    ROS_DEBUG("channel %d ", channel);
    ROS_DEBUG("medianFilter %d ", medianFilter);
    ROS_DEBUG("averageFilter %d", averageFilter);
    ROS_DEBUG("temporalFilterFactor %f", temporalFilterFactor);
    ROS_DEBUG("temporalFilterThreshold %d ", temporalFilterThreshold);
    ROS_DEBUG("edgeFilterThreshold %d", edgeThreshold);
    ROS_DEBUG("temporalEdgeThresholdLow %d ", temporalEdgeThresholdLow);
    ROS_DEBUG("temporalEdgeThresholdHigh %d", temporalEdgeThresholdHigh);
    ROS_DEBUG("interferenceDetectionLimit %d ", interferenceDetectionLimit);
    ROS_DEBUG("interferenceDetectionLimitLastValue %d", useLastValue);
    ROS_DEBUG("cartesian %d", cartesian);
    ROS_DEBUG("publishPointCloud %d", publishPointCloud);

}

void updateConfig(espros_tof_cam660::espros_tof_cam660Config &config, uint32_t level)
{
...
void initialise()
{
    ...
    nh.getParam("lensType", lensType);
    nh.getParam("lensCenterOffsetX", lensCenterOffsetX);
    nh.getParam("lensCenterOffsetY", lensCenterOffsetY);
    nh.getParam("startStream", startStream);
    nh.getParam("imageType", imageType);
    ...
    nh.getParam("minAmplitude", minAmplitude);
    nh.getParam("medianFilter", medianFilter);
    nh.getParam("averageFilter", averageFilter);
    nh.getParam("temporalFilterFactor", temporalFilterFactor);
    nh.getParam("temporalFilterThreshold", temporalFilterThreshold);
    nh.getParam("edgeThreshold", edgeThreshold);
    nh.getParam("temporalEdgeThresholdLow", temporalEdgeThresholdLow);
    nh.getParam("temporalEdgeThresholdHigh", temporalEdgeThresholdHigh);
    nh.getParam("interferenceDetectionLimit", interferenceDetectionLimit);
    nh.getParam("useLastValue", useLastValue);
    nh.getParam("cartesian", cartesian);
    nh.getParam("publishPointCloud", publishPointCloud);
    ...
    ROS_INFO("Camera 660 driver version 1.5");
}
...
{   
    ...
    ros::init(argc, argv, "tof_cam660_node");
    dynamic_reconfigure::Server<espros_tof_cam660::espros_tof_cam660Config> server;
    dynamic_reconfigure::Server<espros_tof_cam660::espros_tof_cam660Config>::CallbackType f;
